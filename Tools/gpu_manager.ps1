...
if ($ListGPUs) {
    ...
    foreach ($gpu in $gpus) {
        ...
        Write-Host "   Memory: $($gpu.MemoryUsed)MB / $($gpu.MemoryTotal)MB ($($gpu.MemoryUsedPercent)% used)" -ForegroundColor Gray
    ...
    if ($optimal) {
        ...
        Write-Host "   Reason: $($optimal.Utilization)% utilization, $($optimal.MemoryFree)MB free memory" -ForegroundColor Gray
    }
    return
}
...
if ($GetOptimalConfig) {
    ...
    $optimal = Get-OptimalGPU -GPUs $gpus
    $config = Get-FFmpegGPUConfig -GPUIndex $optimal.Index -Profile "balanced"
    
    Write-Host "🎯 Selected GPU: $($optimal.Index) ($($optimal.Name))" -ForegroundColor Green
    Write-Host "📊 Status: $($optimal.Utilization)% utilization, $($optimal.MemoryFree)MB free" -ForegroundColor Gray
    Write-Host ""
...
if ($TargetGPU -ge 0) {
    ...
    Write-Host "📊 Status: $($targetGpu.Utilization)% utilization, $($targetGpu.MemoryFree)MB free" -ForegroundColor Gray
    
    $config = Get-FFmpegGPUConfig -GPUIndex $TargetGPU -Profile "balanced"
    Write-Host ""
    Write-Host "🔧 FFmpeg Configuration:" -ForegroundColor Yellow
